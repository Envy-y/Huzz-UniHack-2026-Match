...
function PaymentSuccessContent() {
  ...
  const confirmMutation = trpc.payment.confirmPayment.useMutation()

  useEffect(() => {
    if (playerId && matchId && !confirmed.current) {
...
export default function PaymentSuccessPage() {
  return (
    <PageShell>
      <Suspense fallback={
        <div className="max-w-lg mx-auto px-4 py-16 w-full flex flex-col items-center text-center gap-6">
          <div className="w-24 h-24 rounded-full bg-[#30d5c8]/10 animate-pulse" />
